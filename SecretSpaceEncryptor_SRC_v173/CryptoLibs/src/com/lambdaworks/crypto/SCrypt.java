...
public class SCrypt {
    ...
    static {
        ...
        if(native_library_loaded)
        {
        	...
        	try {
				byte[] testVector = "TestVector".getBytes();
				byte[] testSalt = "TestSalt".getBytes();
				
				byte[] outputNC = scryptN(testVector, testSalt, 128, 8, 1, 128);
				byte[] outputJC = scryptJ(testVector, testSalt, 128, 8, 1, 128);
				
				native_library_loaded = Arrays.equals(outputNC, outputJC);
