...
int modeCode = 0; //PW modification
...
{
	Serpent::ReverseBytes(userKey, keylen); //PW modification
...
{
	Serpent::ReverseBytes(inBlock, 16); //PW modification
	if(modeCode == 1) Serpent::ReverseBytes(outBlock, 16); //PW modification - CTR only
	...
	if(modeCode == 1) Serpent::ReverseBytes(inBlock, 16); //PW modification - CTR only
	Serpent::ReverseBytes(outBlock, 16); //PW modification
}
...
start:
	...
	if(xorBlock)Serpent::ReverseBytes(xorBlock, 16); //PW modification
...
//PW modification
void Serpent::ReverseBytes(const byte *data, size_t len)
{
	unsigned char *lo = (unsigned char*)data;
	unsigned char *hi = (unsigned char*)data + len - 1;
	unsigned char swap;
	while (lo < hi) {
		swap = *lo;
		*lo++ = *hi;
...
void Serpent::setModeCode(int code)
{
	modeCode = code;
}
